...
function PartListNode() {
  ...
  const [chooseBody, setChooseBody] = useState(
    body[Math.floor(Math.random() * (body.length + 1))]
  );
  const [chooseEyes, setChooseEyes] = useState(
    eyes[Math.floor(Math.random() * (eyes.length + 1))]
  );
  const [chooseHair, setChooseHair] = useState(
    hair[Math.floor(Math.random() * (hair.length + 1))]
  );
  const [chooseMouth, setChooseMouth] = useState(
    mouth[Math.floor(Math.random() * (mouth.length + 1))]
  );
  const [chooseEyebrows, setChooseEyebrows] = useState(
    eyebrows[Math.floor(Math.random() * (eyebrows.length + 1))]
  );
  const [chooseHat, setChooseHat] = useState(
    hat[Math.floor(Math.random() * (hat.length + 1))]
  );
  const [chooseGlasses, setChooseGlasses] = useState(
    glasses[Math.floor(Math.random() * (glasses.length + 1))]
  );
  ...
  const handleClick = (item) => {
    ...
    //       console.log(item.props["className"]);
    //       // let newItem = {...item, item.props.className = "portrait choose"}
    //       setBody1(...item);
    //       setChooseBody(item);
    //       return item;
  };
  ...
  const handleRandom = () => {
    setChooseBody(body[Math.floor(Math.random() * (body.length + 1))]);

    setChooseEyes(eyes[Math.floor(Math.random() * (eyes.length + 1))]);
    setChooseHair(hair[Math.floor(Math.random() * (hair.length + 1))]);
    // setChooseMouth(mouth[Math.floor(Math.random() * (mouth.length + 1))]);
    // setChooseEyebrows(
    //   eyebrows[Math.floor(Math.random() * (eyebrows.length + 1))]
    // );
    // setChooseHat(hat[Math.floor(Math.random() * (hat.length + 1))]);
    // setChooseGlasses(glasses[Math.floor(Math.random() * (glasses.length + 1))]);
    // setChooseClothing1(
    //   clothing1[Math.floor(Math.random() * (clothing1.length + 1))]
    // );
    // setChooseClothing2(
        ...
        <div className="containerList">
          <div className="list">
            ...
            <div className="containerPartList ">
              {body1.map((item) => (
                <Img item={item} handleClick={handleClick} choose={item} />
              ))}
            </div>
          </div>
          <div className="list">
            ...
            <div className="containerPartList">
              {eyes1.map((item) => (
                <Img item={item} handleClick={handleClick} />
              ))}
            </div>
          </div>
          <div className="list">
